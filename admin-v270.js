/* Sobre Ruedas Admin v2.9.0 — suspensiones temporales y eliminación definitiva */
(() => {
  ...
  document.title='Sobre Ruedas Admin v2.9.0';
  q('.version').textContent='Admin v2.9.0';
  qa('*').forEach(el=>{
    if(el.childNodes.length===1&&el.firstChild?.nodeType===Node.TEXT_NODE){
      el.textContent=el.textContent.replace(/v2\.5\.1/g,'v2.9.0').replace(/ACTUALIZAR_SUPABASE_v2\.5\.0\.sql/g,'ACTUALIZAR_SUPABASE_v2.9.0.sql');
    }
  });
  ...
  q('#confirmSuspension').onclick=async()=>{
    ...
    if(error){console.error(error);return toast('No se pudo suspender. Ejecuta el SQL v2.9.0')}
  ...
  async function unsuspendPerson(personId,type){
    ...
    if(error){console.error(error);return toast('No se pudo levantar la suspensión. Ejecuta el SQL v2.9.0')}
  ...
  async function permanentDelete(personId,type){
    ...
    if(error){console.error(error);return toast('No se pudo eliminar definitivamente. Ejecuta el SQL v2.9.0')}
...
/* Sobre Ruedas Admin v2.9.0 — soporte, chat y cierre atómico de viajes */
(() => {
  const q=s=>document.querySelector(s);
  const qa=s=>[...document.querySelectorAll(s)];
  const ADMIN_CODE='RUEDA2026';
  ...
  async function loadSupportData(){
    ...
    if(casesResult.error){console.error(casesResult.error);q('#supportCaseList').innerHTML='<div class="empty">No se pudo cargar soporte. Ejecuta el SQL v2.9.0.</div>';return}
  ...
  // v2.9.0 controla la carga y el tiempo real de soporte para evitar renderizados duplicados.
