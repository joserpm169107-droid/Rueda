/* Sobre Ruedas v2.7.0 — perfiles, fotos, vehículo y control de suspensiones */
(() => {
  ...
  document.title=(APP_ROLE==='driver'?'Sobre Ruedas Conductor':'Sobre Ruedas')+' v2.7.0';
  qa('.brand small').forEach(el=>el.textContent='CIEGO DE ÁVILA · v2.7.0');
  qa('p.muted').forEach(el=>{if(/Sobre Ruedas.*v2\.5\.1/.test(el.textContent))el.textContent=(APP_ROLE==='driver'?'Sobre Ruedas Conductor':'Sobre Ruedas')+' v2.7.0 · Ciego de Ávila'});
  ...
  q('#registerBtn').onclick=async()=>{
    ...
    }catch(error){console.error(error);toast('No se pudo subir la foto. Ejecuta el SQL v2.7.0 y vuelve a intentar')}
  ...
  q('#saveProfileBtn').onclick=async()=>{
    ...
    }catch(error){console.error(error);toast('No se pudo guardar el perfil. Revisa el SQL v2.7.0')}
...
/* Sobre Ruedas v2.7.0 — buscador reforzado, chat, ayuda y llamada privada por internet */
(() => {
  ...
  }catch(error){console.warn('Buscador v2.7.0:',error)}
  ...
  async function loadMessages(){
    ...
    if(error){list.innerHTML='<div class="chat-empty">No se pudo cargar el chat. Ejecuta el SQL v2.7.0.</div>';return}
    renderMessages(data||[]);
  ...
  async function submitSupportCase(){
    ...
    if(error){console.error(error);return toast('No se pudo crear el caso. Ejecuta el SQL v2.7.0')}
  ...
  async function loadSupportCases(){
    ...
    if(error){list.innerHTML='<div class="chat-empty">No se pudieron cargar los casos. Ejecuta el SQL v2.7.0.</div>';return}
